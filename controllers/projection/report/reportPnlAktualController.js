...
export const Projection_ReportPnlAktual = async (req, res) => {    
    try {    
        ...
        const volumeSalesTotal = {  
            ...
            ...getVolumeSales.reduce((acc, vs) => { 
                ...
                if (monthName) {  
                    ...
                    const monthActive = `${monthName.toLowerCase().slice(0, 3)}Actual`;
                    acc[monthKey] = roundToTwoDecimalPlaces(vs.total);
                    acc[monthPercent] = 'Ton'  
                    acc[monthActive] = vs.isActual

                    fiscalVolumeSales += vs.total;
                }  
        ...
        const monthlyTotals = {  
            ...
            ...getPenjualanBruto.reduce((acc, vs) => {  
                const monthName = getMonthName(vs.month).toLowerCase();  
                if (monthName) {  
                    const monthKey = `${monthName.slice(0, 3)}Value`;    
                    acc[monthKey] = roundToTwoDecimalPlaces(vs.total);
                    acc[`${monthName.slice(0, 3)}Actual`] = vs.isActual;
        ...
        const getFitting = await fetchComponentData('fitting', jenisProduksiFitting, tahun, domain)
        ...
        const fittingTotal = {  
            ...
            ...getFitting.reduce((acc, vs) => {  
                ...
                if (monthName) {  
                    ...
                    acc[monthKey] = roundToTwoDecimalPlaces(vs.total);
                    acc[monthPercent] = roundToTwoDecimalPlaces(vs.percent); 
                    acc[`${monthName.toLocaleLowerCase().slice(0, 3)}Actual`] = vs.isActual;  
        ...
        const getRuglue = await fetchComponentData('glue', jenisProduksiGlue, tahun, domain)
        ...
        const getOthers = await fetchComponentData('others', jenisProduksiOther, tahun, domain)
        ...
        const getHPP = await fetchHPPData('harga pokok penjualan', jenisProduksiNonProduksi, jenisProduksiCogsTrading, tahun, domain)
        let fiscalHPP = 0;
        const cogsTotal = {  
            index: 9,  
            description: 'HARGA POKOK PENJUALAN',  
            type: 'mainRow',  
            ...getHPP.reduce((acc, vs) => { 
                const monthName = getMonthName(vs.month);  
                if (monthName) {  
                    const monthKey = `${monthName.toLowerCase().slice(0, 3)}Value`;    
                    const monthPercent = `${monthName.toLowerCase().slice(0, 3)}Percent`;
        ...
        const getPendapatanKeuangan = await fetchBebanData("skp/stp/denda pajak", tahun, domain)
...
const fetchVolumeSales = async (tahun, domain) => {  
    const calculateVolumeSales = async (bulan, tahun, domain) => {  
        const results = await db('dbPortalFA.dbo.volume_pnl')  
            .select('bulan')  
            .sum('volume_proyeksi as total_volume_proyeksi')  
            .where('bulan', bulan)  
            .where('tahun', tahun)  
            .where('domain', domain)  
            .groupBy('bulan');
      
        return results.length > 0 ? results[0].total_volume_proyeksi : 0;  
    };  
    
    const results = await db('transaksi_volume_aktual')  
        .select(db.raw('MONTH(ship_date) as month'))  
        .sum('qty as total')  
        .sum('tonase as total_tonase')  
        .where('domain', domain)
        .where('ship_date', '>=', `${tahun}-01-01`)  
        .andWhere('ship_date', '<=', `${tahun}-12-31`)  
        .groupBy(db.raw('MONTH(ship_date)'))  
        .orderBy('month');  
    ...
    results.forEach(result => {  
        ...
        monthlyData[monthIndex].isActual = result.total > 0 ? true : false;
        monthlyData[monthIndex].total = result.total || 0;
        monthlyData[monthIndex].total_tonase = result.total_tonase || 0;
    });

    await Promise.all(
        monthlyData.map(async(data) => {
            if(data.total === 0){
                data.total = await calculateVolumeSales(data.month, tahun, domain);
            }
        })
    )
...
const fetchTaxData = async (component, tahun, domain) => {
    ...
    const aktual = await db('dbPortalFA.dbo.kode_pnl as kp')    
        .select(    
            'kp.kode_pnl_id',    
            db.raw('SUM(ta.jan) as jan'),    
            db.raw('SUM(ta.feb) as feb'),    
            db.raw('SUM(ta.mar) as mar'),    
            db.raw('SUM(ta.apr) as apr'),    
            db.raw('SUM(ta.may) as may'),    
            db.raw('SUM(ta.jun) as jun'),    
            db.raw('SUM(ta.jul) as jul'),    
            db.raw('SUM(ta.aug) as aug'),    
            db.raw('SUM(ta.sep) as sep'),    
            db.raw('SUM(ta.oct) as oct'),    
            db.raw('SUM(ta.nov) as nov'),    
            db.raw('SUM(ta.dec) as dec')    
        )    
        .join('dbPortalFA.dbo.pnl_gl as pnl', 'kp.kode_pnl_id', 'pnl.kode_pnl_id')
        .join('dbPortalFA.dbo.transaksi_aktual as ta', 'pnl.gl_id', 'ta.gl_code') 
        .whereRaw('LOWER(kp.detail) = ?', [component]) 
    ...
    for (let i = 1; i <= 12; i++) {  
        ...
        const total = aktual.reduce((sum, record) => sum + (record[monthName] || 0), 0);   
        monthlyTotals.push({ month: i, total, isActual: true });  
    }
    
    await Promise.all(
        monthlyTotals.map(async(data) => {
            if(data.total === 0) {
                data.isActual = false;
                data.total = await calculateTax(component, tahun, domain);  
            }
        })
    )
...
const fetchBebanData = async(beban, tahun, domain) => {
    ...
    const aktual = await db('dbPortalFA.dbo.kode_pnl as kp')    
        ...
        .join('dbPortalFA.dbo.pnl_gl as pnl', 'kp.kode_pnl_id', 'pnl.kode_pnl_id')
        .join('dbPortalFA.dbo.transaksi_aktual as ta', 'pnl.gl_id', 'ta.gl_code') 
        .whereRaw('LOWER(kp.detail) = ?', [beban]) 
        .where('kp.tipe', 'Aktual')    
        .where('ta.domain', domain)
        .where('ta.tahun', tahun)
        .groupBy('kp.kode_pnl_id'); 
    ...
    await Promise.all(
        monthlyTotals.map(async(data) => {
            if(data.total === 0) {
                data.isActual = false;
                data.total = await calculateBeban(beban, data.month, tahun, domain); 
            } else {
...
const fetchComponentData= async(transaksiAktual, jenisProduksiComp, tahun, domain) => { 
    const fetchVolumeAndPriceData = async (jenisProduksiList, bulan, tahun, domain) => {  
        const results = await db('dbPortalFA.dbo.volume_pnl as v')  
            ...
            .select('v.volume_budget as volume', 'a.avg_price_proyeksi as price', 'a.diskon_proyeksi as diskon', 'v.jenis_produksi_id')  
    ...
    const aktual = await db('dbPortalFA.dbo.kode_pnl as kp')    
        ...
        .join('dbPortalFA.dbo.pnl_gl as pnl', 'kp.kode_pnl_id', 'pnl.kode_pnl_id')
        .join('dbPortalFA.dbo.transaksi_aktual as ta', 'pnl.gl_id', 'ta.gl_code') 
        .whereRaw('LOWER(kp.detail) = ?', [transaksiAktual]) 
        .where('kp.tipe', 'Aktual')    
        .where('ta.domain', domain)
        .where('ta.tahun', tahun)
        .groupBy('kp.kode_pnl_id'); 
    ...
    await Promise.all(
        monthlyTotals.map(async(data) => {
            if(data.total === 0) {
                const bruto = await calculatePenjualanBruto(jenisProduksiComp, data.month, tahun, domain);
                const potongan = await calculatePotonganPenjualan(jenisProduksiComp, data.month, tahun, domain);  
...
const fetchFOHData = async (tahun, domain) => {
    const checkActualTransaction = async (tahun, domain) => {  
        const aktual = await db('dbPortalFA.dbo.kode_pnl as kp')    
            .select(    
                'kp.kode_pnl_id',    
                db.raw('SUM(ta.jan) as jan'),    
        ...
        await Promise.all(
            monthlyTotals.map(async(data) => {
                if(data.total === 0) {
                    data.isActual = false;
                    data.total = await calculateFOH(data.month, tahun, domain);
                } else {
                    data.total = 1 * -data.total;
                }
            })
    ...
    const calculateFOH = async (bulan, tahun, domain) => {    
        try {    
            const result = await db('dbPortalFA.dbo.opex')    
                ...
                .where('tahun', tahun)  
                .where('domain', domain)  
                .sum({ totalProyeksi: 'proyeksi' });  
               
            const totalProyeksi = result[0]?.totalProyeksi || 0;
            const valueOPEX = totalProyeksi * -1;    
    ...
    let fohTotal = await checkActualTransaction(tahun, domain);
    return fohTotal; 
}

const fetchHPPData = async (transaksiAktual, jenisProduksiCogsProject, jenisProduksiCogsTrading, tahun, domain) => {
    const fetchVolumeAndPriceData = async (jenisProduksiList, bulan, tahun, domain) => {  
        const results = await db('dbPortalFA.dbo.volume_pnl as v')  
            .join('dbPortalFA.dbo.asumsi_price_pnl as a', function() {  
                this.on('v.jenis_produksi_id', '=', 'a.jenis_produksi_id')  
            ...
            .select('v.volume_proyeksi as volume', 'a.hpp_proyeksi as hpp', 'v.jenis_produksi_id')  
            .whereIn('v.jenis_produksi_id', jenisProduksiList.map(jenis => jenis.jenis_produksi_id))  
            .where('v.bulan', bulan)  
            .where('v.tahun', tahun)  
            .andWhere('v.domain', domain);  
    ...
    const calculateBahanPembantu = async (bulan, tahun, domain) => {
        ...
        const volume = await db('dbPortalFA.dbo.volume_produksi_pnl')  
          ...
          .sum('volume_proyeksi as total_volume_budget')  
          .first();  
    ...
    const calculateRawMaterial = async (bulan, tahun, domain) => {
        ...
        .select(  
            db.raw('SUM(vpp.volume_proyeksi) AS volume_proyeksi'),  
            db.raw('SUM(vpp.volume_proyeksi * r.value_reject) AS total'),  
            db.raw(`  
                COALESCE(  
    ...
    await Promise.all(
        monthlyTotals.map(async(data) => {
            if(data.total === 0) {
                const cogsProject = await calculateCOGS(jenisProduksiCogsProject, data.month, tahun, domain);  
                const cogsTrading = await calculateCOGS(jenisProduksiCogsTrading, data.month, tahun, domain);

                // Count Available Sales
                const rawmaterial = await calculateRawMaterial(data.month, tahun, domain);
                const bahanPembantu = await calculateBahanPembantu(data.month, tahun, domain);
                const foh = await calculateFOH(data.month, tahun, domain);
                const box = await calculateBox(data.month, tahun, domain);
                const beginingInventory = await getBeginingInventory(data.month, tahun, domain);
                const availableSales = beginingInventory + (rawmaterial + bahanPembantu + foh + box);
                
                // Count Cogs
                const totalSumJenisProduksi = await calculateSumJenisProduksi(data.month, tahun, domain);
                const totalSumVolumeBudget = await calculateVolumeBudget(data.month, tahun, domain);
...
const fetchBoxData = async (glAccounts, tahun, domain) => {
    const checkActualTransaction = async (glAccounts, tahun, domain) => {  
            const aktual = await db('dbPortalFA.dbo.kode_pnl as kp')    
            .select(    
                'kp.kode_pnl_id',    
                db.raw('SUM(ta.jan) as jan'),    
            ...
            .join('dbPortalFA.dbo.pnl_gl as pnl', 'kp.kode_pnl_id', 'pnl.kode_pnl_id')
            .join('dbPortalFA.dbo.transaksi_aktual as ta', 'pnl.gl_id', 'ta.gl_code') 
            .whereRaw('LOWER(kp.detail) = ?', ['box/metal insert/karet']) 
            .where('kp.tipe', 'Aktual')    
            .where('ta.domain', domain)
            .where('ta.tahun', tahun)
            .groupBy('kp.kode_pnl_id');
        ...
        await Promise.all(
            monthlyTotals.map(async(data) => {
                if(data.total === 0) {
                    data.isActual = false;
                    data.total = await calculateValueOPEX(glAccounts, data.month, tahun, domain);
                }
            })
        )
    ...
    const boxTotal = await checkActualTransaction(glAccounts, tahun, domain);
    return boxTotal; 
}

const fetchTradingData = async (jenisProduksiCogsTrading, tahun, domain) => {
    const checkActualTransaction = async (jenisProduksiCogsTrading, tahun, domain) => {  
        const aktual = await db('dbPortalFA.dbo.kode_pnl as kp')    
            .select(    
                'kp.kode_pnl_id',    
                db.raw('SUM(ta.jan) as jan'),    
            ...
            .join('dbPortalFA.dbo.pnl_gl as pnl', 'kp.kode_pnl_id', 'pnl.kode_pnl_id')
            .join('dbPortalFA.dbo.transaksi_aktual as ta', 'pnl.gl_id', 'ta.gl_code') 
            .whereRaw('LOWER(kp.detail) = ?', ['trading']) 
            .where('kp.tipe', 'Aktual')    
            .where('ta.domain', domain)
            .where('ta.tahun', tahun)
            .groupBy('kp.kode_pnl_id'); 
        ...
        await Promise.all(
            monthlyTotals.map(async(data) => {
                if(data.total === 0) {
                    data.isActual = false;
                    data.total = await calculateCOGS(jenisProduksiCogsTrading, data.month, tahun, domain);
                    data.percent = 0;
                } else {
                    data.total = 1 * -data.total;
                }
    ...
    const tradingTotal = await checkActualTransaction(jenisProduksiCogsTrading, tahun, domain)
    return tradingTotal;
}

const fetchPipaData = async(jenisProduksiPipa, jenisProduksiPipaNonProduksi, tahun, domain) => {
    const checkActualTransaction = async (jenisProduksiPipa, jenisProduksiPipaNonProduksi, tahun, domain) => {  
        const aktual = await db('dbPortalFA.dbo.kode_pnl as kp')    
            .select(    
                'kp.kode_pnl_id',    
                db.raw('SUM(ta.jan) as jan'),    
        ...
        for (let i = 1; i <= 12; i++) {  
            ...
            const total = aktual.reduce((sum, record) => sum + (record[monthName] || 0), 0);   
            monthlyTotals.push({ month: i, total });  
        }
      
        await Promise.all(
            monthlyTotals.map(async(data) => {
                if(data.total === 0) {
                    const brutoPipa = await calculatePenjualanBruto(jenisProduksiPipa, data.month, tahun, domain);  
                    const brutoNonProduksi = await calculatePenjualanBruto(jenisProduksiPipaNonProduksi, data.month, tahun, domain);  
              
                    const potonganPipa = await calculatePotonganPenjualan(jenisProduksiPipa, data.month, tahun, domain);  
                    const potonganNonProduksi = await calculatePotonganPenjualan(jenisProduksiPipaNonProduksi, data.month, tahun, domain);  
              
                    const penjualanBersih = (brutoPipa + brutoNonProduksi) - (potonganPipa + potonganNonProduksi);  
                    data.total = brutoPipa + brutoNonProduksi + potonganPipa + potonganNonProduksi;  
                    data.percent = penjualanBersih !== 0 ? (data.total / penjualanBersih) : 0;
                } else {
                    data.total = 1 * -data.total;
                }
            })
    ...
    const fetchVolumeAndPriceData = async (jenisProduksiList, bulan, tahun, domain) => {  
        ...
        const results = await db('dbPortalFA.dbo.volume_pnl as v')  
            ...
            .select('v.volume_proyeksi as volume', 'a.avg_price_proyeksi as price', 'a.diskon_proyeksi as diskon', 'v.jenis_produksi_id')  
            .whereIn('v.jenis_produksi_id', jenisProduksiList.map(jenis => jenis.jenis_produksi_id))  
            .where('v.bulan', bulan)  
            .where('v.tahun', tahun)  
            .andWhere('v.domain', domain);  
    ...
    const pipaTotal = await checkActualTransaction(jenisProduksiPipa, jenisProduksiPipaNonProduksi, tahun, domain);
    return pipaTotal;   
}

const fetchPotonganPenjualan = async (jenisProduksi, jenisProduksiProject, jenisProduksiNonProduksi, tahun, domain) => {
    const checkActualTransaction = async (jenisProduksi, jenisProduksiProject, jenisProduksiNonProduksi, tahun, domain) => {  
        const aktual = await db('dbPortalFA.dbo.kode_pnl as kp')    
            .select(    
                'kp.kode_pnl_id',    
                db.raw('SUM(ta.jan) as jan'),    
            ...
            .join('dbPortalFA.dbo.pnl_gl as pnl', 'kp.kode_pnl_id', 'pnl.kode_pnl_id')
            .join('dbPortalFA.dbo.transaksi_aktual as ta', 'pnl.gl_id', 'ta.gl_code')   
            .whereRaw('LOWER(kp.detail) = ?', ['potongan penjualan']) 
            .where('kp.tipe', 'Aktual')    
            .where('ta.domain', domain)
            .where('ta.tahun', tahun)
            .groupBy('kp.kode_pnl_id');  
        ...
        await Promise.all(
            monthlyTotals.map(async(data) => {
                if(data.total === 0) {
                    const retailTotal = await calculatePotonganPenjualan(jenisProduksi, data.month, tahun, domain);  
                    const projectTotal = await calculatePotonganPenjualan(jenisProduksiProject, data.month, tahun, domain);  
                    const nonProduksiTotal = await calculatePotonganPenjualan(jenisProduksiNonProduksi, data.month, tahun, domain);  
        
                    data.isActual = false;
                    data.total = retailTotal + projectTotal + nonProduksiTotal;
                }
            })
        )
    ...
    const potonganPenjualanTotal = await checkActualTransaction(jenisProduksi, jenisProduksiProject, jenisProduksiNonProduksi, tahun, domain)
...
const fetchPenjualanBruto = async (jenisProduksi, jenisProduksiProject, jenisProduksiNonProduksi, tahun, domain) => {
    const checkActualTransaction = async (jenisProduksi, jenisProduksiProject, jenisProduksiNonProduksi, tahun, domain) => {  
        const aktual = await db('dbPortalFA.dbo.kode_pnl as kp')    
            ...
            .join('dbPortalFA.dbo.pnl_gl as pnl', 'kp.kode_pnl_id', 'pnl.kode_pnl_id')
            .join('dbPortalFA.dbo.transaksi_aktual as ta', 'pnl.gl_id', 'ta.gl_code') 
            .whereRaw('LOWER(kp.detail) = ?', ['penjualan bruto']) 
            .where('kp.tipe', 'Aktual')    
            .where('ta.domain', domain)
            .where('ta.tahun', tahun)
            .groupBy('kp.kode_pnl_id');  
        ...
        await Promise.all(
            monthlyTotals.map(async(data) => {
                if(data.total === 0) {
                    const retailTotal = await calculateTotal(jenisProduksi, data.month, tahun, domain);  
                    const projectTotal = await calculateTotal(jenisProduksiProject, data.month, tahun, domain);  
                    const nonProduksiTotal = await calculateTotal(jenisProduksiNonProduksi, data.month, tahun, domain);  
    ...
    const calculateTotal = async (jenisProduksiList, bulan, tahun, domain) => {
        const results = await db('dbPortalFA.dbo.volume_pnl as v')  
            ...
            .select('v.volume_proyeksi as volume', 'a.avg_price_proyeksi as price', 'v.jenis_produksi_id')  
            .whereIn('v.jenis_produksi_id', jenisProduksiList.map(jenis => jenis.jenis_produksi_id))  
            .where('v.bulan', bulan)  
            .where('v.tahun', tahun)  
            .andWhere('v.domain', domain);  
    ...
    let monthlyTotals = await checkActualTransaction(jenisProduksi, jenisProduksiProject, jenisProduksiNonProduksi, tahun, domain);
    return monthlyTotals;  
}
